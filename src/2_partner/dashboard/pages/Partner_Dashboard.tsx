...
//   3. Live Orders navLink is now a button, not a Link
//   4. Main content switches between Outlet and PartnerOrders
// ============================================================
...
const PartnerDashboardLayout = () => {
    ...
    const { restaurantName, logoUrl, isLoading } = useRestaurant();
        ...
        const checkSetup = async () => {
            try {
                ...
                const { data: restaurant } = await supabase
                    .from('restaurants').select('name').eq('owner_id', user.id)
    ...
    const fetchProfile = async () => {
        try {
            ...
            if (restaurantData) setRestaurantId((restaurantData as any).id);
    ...
    useEffect(() => {
        ...
        partnerOrderService.getOrderStats(restaurantId || '').then(s => {
    ...
    return (
        <div
            className={`min-h-screen transition-colors duration-500 partner-dashboard ${isDarkMode ? 'dark' : ''} bg-[var(--dash-bg)]`}
            ...
            `}>
                <div className="p-6 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[var(--primary)] to-yellow-600 p-[2px] shadow-[0_0_15px_rgba(244,175,37,0.3)] shrink-0">
                        {isLoading ? (
                            <div className="w-full h-full rounded-full bg-white/10 animate-pulse border-2 border-[var(--dash-bg)]" />
                        ) : logoUrl ? (
                            <img src={logoUrl} className="w-full h-full rounded-full object-cover border-2 border-[var(--dash-bg)]" alt="Logo" />
                        ) : (
                            <div className="w-full h-full rounded-full bg-[var(--background-dark)] flex items-center justify-center border-2 border-[var(--dash-bg)] text-white/50 text-xs font-bold uppercase">
                                ...
                                {link.badge && (
                                    <span className={`
                                        ...
                                        ${link.badge === 'AI'
                                            ...
                                            : 'px-2 py-[2px] rounded-full bg-red-500 text-white text-[10px] font-black shadow-[0_0_8px_rgba(239,68,68,0.8)]'
                                        }
                                    `}>
                                        {link.badge}
                                    </span>
                ...
                <div className="p-4 mt-auto">
                    ...
                    >
                        ...
                        <span className="tracking-wide">{t('dash.signOut')}</span>
                    </button>
                </div>
            </aside>
...
export const DashboardOverview = () => {
    ...
    const formatWithRestaurantCurrency = (value: number | string) => {
        const parsed = typeof value === 'string' ? parseFloat(value) : value;
        const safe = Number.isFinite(parsed) ? parsed : 0;
        const locale = currencyCode === 'PKR' ? 'en-PK' : 'en-US';
        return `${currencySymbol} ${new Intl.NumberFormat(locale, { maximumFractionDigits: 0 }).format(safe)}`;
    };
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const { data: restaurant } = await supabase.from('restaurants').select('id, is_open').eq('owner_id', user.id).maybeSingle();
            ...
            if (restaurant) {
                ...
                if (isDemoMode) {
                    ...
                    const { data: rpcData, error: rpcError } = await (supabase as any).rpc('get_dashboard_stats', { p_restaurant_id: restaurantId });
                    if (!rpcError && rpcData) {
                        setStats({
    ...
    useEffect(() => { fetchDashboardData(); }, [isDemoMode]);

    const toggleStatus = async () => {
        const newStatus = !isOnline;
        setIsOnline(newStatus);
        const { data: { user } } = await supabase.auth.getUser();
        if (user) await (supabase as any).from('restaurants').update({ is_open: newStatus }).eq('owner_id', user.id);
    };

    const statCards = [
        {
            title: "Today's Revenue",
            value: formatWithRestaurantCurrency(stats.revenue),
            subtext: (
                <div className="flex flex-col gap-0.5 mt-1">
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                        <span>Cash & COD:</span>
                        <span className="font-bold text-slate-600">{formatWithRestaurantCurrency(stats.cash_revenue)}</span>
                    </div>
                ...
                <div className="bg-white/80 backdrop-blur-xl p-6 rounded-2xl border border-white/40 shadow-xl hover:shadow-2xl transition-all duration-300 group h-full relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`p-3 bg-gradient-to-br from-amber-100 to-amber-50 rounded-xl shadow-inner group-hover:scale-110 transition-transform duration-300`}>
                                <Icon className="text-amber-600 drop-shadow-sm" size={24} />
                            </div>
                            {trend !== null && (
        ...
        <div className="space-y-8 animate-in fade-in duration-700 relative min-h-screen pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/70 backdrop-blur-lg p-6 rounded-3xl border border-white/50 shadow-lg relative overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div onClick={toggleStatus} className={`relative z-10 cursor-pointer flex items-center gap-4 px-2 py-2 pr-6 rounded-full transition-all duration-500 border-2 ${isOnline ? 'bg-green-50/80 border-green-200 shadow-lg' : 'bg-slate-50/80 border-slate-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((stat, idx) => <StatsCardInner key={idx} {...stat} />)}
            </div>
