...
# The new handler will use a single-image horizontal spread with CORB fixes
new_handle_download = r'''    const handleDownloadMenu = async () => {
        ...
        try {
            const exportToast = toast.loading("Generating Multi-Page Horizontal Spread (Safe Mode)...");

            // Helper: Slugify (Bulletproof V16)
            ...
            // Capture Mosaic Images for deals (already localized in DOM)
            ...
            const categoryCardsHtml = Object.keys(grouped).map(cat => {
                const itemsList = grouped[cat].map((item: any) => {
                    const imgSrc = getSafeImg(getRealImgSrc(item));
                    return `
                        <div style="margin-bottom: 40px; border-bottom: 3px dashed ${styles.border}; padding-bottom: 30px; display: flex; align-items: center; gap: 35px;">
                            ${imgSrc ? `<div style="width: 90px; height: 90px; border-radius: 20px; overflow: hidden; border: 4px solid ${styles.accent}; flex-shrink: 0;"><img src="${imgSrc}" crossorigin="anonymous" style="width: 100%; height: 100%; object-fit: cover;"></div>` : ''}
                            <div style="flex: 1;">
            ...
            toast.loading("Resolving Assets & Rendering UHD Spread...", { id: exportToast });
            ...
            link.download = `${slugify(restaurantInfo.name)}_Master_UHD_Spread.${ext}`;
            link.click();

            document.body.removeChild(ghost);
            toast.dismiss(exportToast);
            toast.success("Single Wide UHD Master Spread Downloaded! 📥🗞️💎");

        } catch (error) {
            console.error("Export error:", error);
            toast.error("Spread Generation Failed");
        }
    };
'''
...
print("Successfully applied CORB/CORS fixes and Wide Spread Polish to MenuManager Export")
