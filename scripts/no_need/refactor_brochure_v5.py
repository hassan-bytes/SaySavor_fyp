...
new_handle_download = r'''    const handleDownloadMenu = async () => {
        ...
        try {
            ...
            if (deals.length > 0) {
                ...
                // Robust Capture: Priority to DB image_url
            ...
            // 4. Construct Ghost Element (LANDSCAPE SPREAD V5 - Ultra Clarity)
            ...
            const headerHtml = `
                <div style="background: ${styles.darkerBg}; padding: 100px; text-align: center; border-bottom: 12px solid ${styles.accent}; position: relative; overflow: hidden;">
                    <h1 style="font-size: 150px; font-weight: 950; color: ${styles.headerText}; margin: 0; text-transform: uppercase; letter-spacing: 30px; line-height: 0.9; filter: drop-shadow(0 10px 20px rgba(0,0,0,0.3));">${restaurantInfo.name}</h1>
                    ${exportOptions.includeDescription ? `<div style="font-size: 36px; color: ${styles.accent}; margin-top: 30px; font-weight: 700; font-style: italic; letter-spacing: 5px; opacity: 0.9;">${restaurantInfo.description}</div>` : ''}
                    
                    ${exportOptions.includeLocation ? `
                        <div style="margin-top: 60px; display: inline-flex; justify-content: center; align-items: center; gap: 100px; font-size: 32px; color: ${styles.text}; font-weight: 900; padding: 30px 80px; background: rgba(255,255,255,0.08); border-radius: 100px; border: 3px solid rgba(255,255,255,0.15); backdrop-filter: blur(20px);">
            ...
            if (deals.length > 0) {
                const dealCards = deals.map((deal: any) => {
                    ...
                    return `
                        <div style="width: calc(33.33% - 27px); background: ${styles.dealGradient}; padding: 50px; border-radius: 60px; border: 5px solid ${styles.accent}; display: flex; gap: 40px; align-items: center; box-shadow: 0 40px 80px rgba(0,0,0,0.5); position: relative; overflow: hidden;">
                            <div style="position: absolute; top: -15px; right: -15px; background: ${styles.accent}; color: ${styles.bg.includes('f') ? '#000' : '#fff'}; padding: 15px 50px; font-weight: 950; font-size: 22px; transform: rotate(15deg); box-shadow: 0 10px 25px rgba(0,0,0,0.4); border: 2px solid white;">EXCLUSIVE</div>
                            <div style="width: 240px; height: 240px; border-radius: 40px; overflow: hidden; border: 8px solid #fff; background: #222; flex-shrink: 0; box-shadow: 0 15px 30px rgba(0,0,0,0.3);">
                                <img src="${src}" style="width: 100%; height: 100%; object-fit: cover;"> 
                            </div>
                            <div style="flex: 1;">
                                <div style="font-size: 40px; font-weight: 950; color: #fff; margin-bottom: 20px; text-transform: uppercase; line-height: 1.1; letter-spacing: 1px;">${deal.name}</div>
                ...
                dealsHtml = `
                    <div style="padding: 120px 100px 80px 100px; background: ${styles.bg};">
                        <div style="font-weight: 950; font-size: 45px; color: ${styles.accent}; margin-bottom: 60px; text-transform: uppercase; letter-spacing: 15px; display: flex; align-items: center; gap: 40px;">
                            <div style="height: 6px; flex: 1; background: ${styles.accent}; border-radius: 3px;"></div>
                            PREMIUM SELECTIONS
                            <div style="height: 6px; flex: 1; background: ${styles.accent}; border-radius: 3px;"></div>
                        </div>
            ...
            const categoryCards = Object.keys(grouped).map(cat => {
                const itemsList = grouped[cat].map((item: any) => {
                    ...
                    return `
                        <div style="margin-bottom: 30px; border-bottom: 2px dashed ${styles.border}; padding-bottom: 25px; display: flex; align-items: flex-start; gap: 25px;">
                            ${imgSrc ? `
                            <div style="width: 75px; height: 75px; border-radius: 15px; overflow: hidden; border: 3px solid ${styles.accent}; flex-shrink: 0; shadow: 0 4px 10px rgba(0,0,0,0.1);">
                ...
                return `
                    <div style="background: ${styles.cardBg}; border-radius: 50px; overflow: hidden; border: 4px solid ${styles.border}; margin-bottom: 50px; box-shadow: 0 25px 60px rgba(0,0,0,0.1); break-inside: avoid; display: flex; flex-direction: column;">
                        ${catImgSrc ? `<div style="height: 450px; width: 100%; overflow: hidden; background: #000;"><img src="${catImgSrc}" style="width: 100%; height: 100%; object-fit: cover; border-bottom: 10px solid ${styles.accent}"></div>` : ''}
                        <div style="padding: 50px;">
                            <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 40px;">
                                <div style="height: 6px; flex: 1; background: ${styles.accent}; border-radius: 6px;"></div>
                                <div style="font-weight: 950; font-size: 38px; text-transform: uppercase; color: ${styles.accent}; letter-spacing: 5px;">${cat}</div>
                                <div style="height: 6px; flex: 1; background: ${styles.accent}; border-radius: 6px;"></div>
                            </div>
                            <div>${itemsList}</div>
                        </div>
            ...
            const footerHtml = `
                <div style="background: ${styles.darkerBg}; padding: 100px; text-align: center; border-top: 8px solid ${styles.accent};">
                    <div style="font-size: 32px; color: ${styles.mutedText}; font-weight: 900; letter-spacing: 12px; text-transform: uppercase; margin-bottom: 25px;">EXPERIENCE PERFECTION AT ${restaurantInfo.name}</div>
                    <div style="font-size: 18px; color: ${styles.mutedText}; opacity: 0.5; font-weight: 600;">Handcrafted Menu • Powered by SaySavor Pro AI Suite</div>
                </div>
            `;
            ...
            // 5. Capture & Save
            toast.loading("Rendering ultra-sharp UHD spread...", { id: exportToast });
            await new Promise(resolve => setTimeout(resolve, 5000)); 

            const canvas = await html2canvas(ghost, {
                scale: 2, // 4800px wide for Ultra HD Sharpness
            ...
            toast.success("Ultra-HD Brochure Downloaded! 📥✨");

        } catch (error) {
            console.error("Export error:", error);
            toast.error("Failed to generate UHD spread");
        }
    };
'''
...
print("Successfully applied Image Quality and Deal Correctness Fixes")
