...
# The new handler will use a horizontal spread system
new_handle_download = r'''    const handleDownloadMenu = async () => {
        ...
        try {
            const exportToast = toast.loading("Generating Multi-Page Horizontal Spread...");

            // Helper: Slugify (Bulletproof V15)
            ...
            if (deals.length > 0) {
                toast.loading("Integrating Deals into Spread...", { id: exportToast });
            ...
            const dealCardsHtml = deals.map(deal => {
                ...
                return `
                    <div style="width: 100%; background: ${styles.dealGradient}; padding: 60px; border-radius: 65px; border: 8px solid ${styles.accent}; display: flex; gap: 50px; align-items: flex-start; box-shadow: 0 50px 100px rgba(0,0,0,0.6); margin-bottom: 60px; break-inside: avoid;">
                        ...
                        <div style="flex: 1;">
                            <div style="font-size: 60px; font-weight: 950; color: #fff; margin-bottom: 25px; text-transform: uppercase; letter-spacing: 4px;">${deal.name}</div>
                ...
                const itemsList = grouped[cat].map((item: any) => {
                    ...
                    return `
                        <div style="margin-bottom: 40px; border-bottom: 3px dashed ${styles.border}; padding-bottom: 30px; display: flex; align-items: center; gap: 35px;">
                            ${imgSrc ? `<div style="width: 90px; height: 90px; border-radius: 20px; overflow: hidden; border: 4px solid ${styles.accent}; flex-shrink: 0;"><img src="${imgSrc}" style="width: 100%; height: 100%; object-fit: cover;"></div>` : ''}
                            <div style="flex: 1;">
            ...
            // Combine All Cards for Flow
            const allContentCards = [...dealCardsHtml, ...categoryCardsHtml];
            
            // Distribute Cards into Pages
            // Approx logic: Based on character count or just simple chunking (Deals are big, categories vary)
            // Let's do 2-3 categories/deals per column.
            const pages: string[][] = [[]];
            let currentContentSize = 0;
            const MAX_PER_PAGE = 3; // Start with 3 large chunks per page (since Deals are huge now)

            allContentCards.forEach((card, idx) => {
                const pageIdx = pages.length - 1;
                pages[pageIdx].push(card);
                if (pages[pageIdx].length >= MAX_PER_PAGE && idx < allContentCards.length - 1) {
                    pages.push([]);
                }
            });

            // 3. Create Wide Ghost Container
            ...
            const pageHtml = pages.map((pageContent, idx) => {
                const header = idx === 0 ? `
                    <div style="background: ${styles.darkerBg}; padding: 70px; text-align: center; border-bottom: 25px solid ${styles.accent}; margin-bottom: 60px;">
                        <h1 style="font-size: 150px; font-weight: 950; color: ${styles.headerText}; margin: 0; text-transform: uppercase; letter-spacing: 45px; line-height: 0.8;">${restaurantInfo.name}</h1>
                        <div style="font-size: 44px; color: ${styles.accent}; margin-top: 35px; font-weight: 800; font-style: italic; letter-spacing: 12px;">${restaurantInfo.description || 'MASTER CUISINE'}</div>
                    </div>
                ` : `
                    <div style="padding: 60px 80px; text-align: left; border-bottom: 8px solid ${styles.accent}; margin-bottom: 60px; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div style="font-size: 30px; color: ${styles.mutedText}; font-weight: 800; letter-spacing: 8px;">CONTINUED</div>
                    </div>
                `;

                return `
                    <div style="width: ${A4_WIDTH}px; height: ${A4_HEIGHT}px; border-right: 4px solid ${styles.accent}; display: flex; flex-direction: column; overflow: hidden; position: relative;">
                        ${idx === 0 ? header : ''}
            ...
            toast.loading("Rendering Ultra-Wide Master Spread (300 DPI)...", { id: exportToast });
            await new Promise(r => setTimeout(r, 12000)); // Long wait for huge width

            const canvas = await html2canvas(ghost, {
                scale: 1, // 3508px is already huge, scale 1 is enough for UHD
                backgroundColor: styles.bg, useCORS: true, allowTaint: true, logging: false
            } as any);
            ...
            link.download = `${slugify(restaurantInfo.name)}_Master_A4_Spread.${ext}`;
            link.click();

            document.body.removeChild(ghost);
            toast.dismiss(exportToast);
            toast.success("Single Wide Master Spread Exported! 📥🗞️💎");

        } catch (error) {
            console.error("Export error:", error);
            toast.error("Failed to generate wide spread");
        }
    };
'''
...
print("Successfully implemented Single-Image Wide A4 Horizontal Spread")
