...
new_handle_download = r'''    const handleDownloadMenu = async () => {
        ...
        try {
            ...
            // Helper: Slugify (BULLETPROOF V13 - Matching DynamicFoodImage)
            const slugify = (text: string) => {
                return text
                    .trim()
                    .toLowerCase()
                    .replace(/[()]/g, '')
                    .replace(/\./g, '-') // Preserve decimals as hyphens
                    .replace(/\s+/g, '-')
                    .replace(/ñ/g, 'n') // Normalize special char
                    .replace(/[^a-z0-9-]/g, '')
                    .replace(/-+/g, '-')
                    .replace(/^-+|-+$/g, '');
            };

            // Helper: Get Image Source (SYNCED EDITION)
            const getRealImgSrc = (item: any) => {
                ...
                if (item.cuisine && item.category && item.name) {
                    ...
                    // Strategy B: Slugified
                    const pathB = `${cleanCuisine}/${cleanCategory}/${slugify(cleanName)}.jpg`;
                    const { data: urlB } = supabase.storage.from('preset-images').getPublicUrl(pathB);

                    // Strategy C: Normalized (ñ -> n)
                    const pathC = `${cleanCuisine}/${cleanCategory}/${cleanName.replace(/ñ/g, 'n')}.jpg`;
                    const { data: urlC } = supabase.storage.from('preset-images').getPublicUrl(pathC);
...
print("Successfully synchronized Resilient Asset Sync to MenuManager Export")
