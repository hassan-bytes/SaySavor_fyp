...
new_handle_download = r'''    const handleDownloadMenu = async () => {
        ...
        try {
            ...
            // Helper: Get Image Source (PRIORITY: LIVE UI)
            const getRealImgSrc = (item: any) => {
                // 1. Try Live DOM (Exactly what user sees)
            ...
            if (deals.length > 0) {
                toast.loading("Matching your visual identity...", { id: exportToast });
                for (const deal of deals) {
                    // CRITICAL: Look in UI first
                    const uiImg = document.querySelector(`[data-deal-id="${deal.id}"] img`) as HTMLImageElement;
                    if (uiImg && uiImg.src && uiImg.src.startsWith('http')) {
                        dealImages[deal.id] = uiImg.src;
            ...
            // 4. Construct Ghost Element (LANDSCAPE SPREAD V6 - Balanced Heights)
            ...
            const headerHtml = `
                <div style="background: ${styles.darkerBg}; padding: 80px 100px; text-align: center; border-bottom: 12px solid ${styles.accent}; position: relative; overflow: hidden;">
                    <h1 style="font-size: 140px; font-weight: 950; color: ${styles.headerText}; margin: 0; text-transform: uppercase; letter-spacing: 25px; line-height: 0.9; filter: drop-shadow(0 8px 15px rgba(0,0,0,0.3));">${restaurantInfo.name}</h1>
                    ${exportOptions.includeDescription ? `<div style="font-size: 32px; color: ${styles.accent}; margin-top: 25px; font-weight: 700; font-style: italic; letter-spacing: 5px; opacity: 0.9;">${restaurantInfo.description}</div>` : ''}
                    
                    ${exportOptions.includeLocation ? `
                        <div style="margin-top: 50px; display: inline-flex; justify-content: center; align-items: center; gap: 80px; font-size: 28px; color: ${styles.text}; font-weight: 900; padding: 25px 60px; background: rgba(255,255,255,0.06); border-radius: 100px; border: 2px solid rgba(255,255,255,0.12); backdrop-filter: blur(15px);">
                            <span style="display: flex; align-items: center; gap: 15px;">📍 <span style="border-bottom: 3px solid ${styles.accent}">${restaurantInfo.address || 'Join Us'}</span></span>
                            <span style="display: flex; align-items: center; gap: 15px;">📞 <span style="border-bottom: 3px solid ${styles.accent}">${restaurantInfo.phone || 'Contact'}</span></span>
                        </div>
                    ` : ''}
                </div>
            `;
            ...
            if (deals.length > 0) {
                const dealCards = deals.map((deal: any) => {
                    ...
                    return `
                        <div style="width: calc(33.33% - 27px); background: ${styles.dealGradient}; padding: 45px; border-radius: 50px; border: 4px solid ${styles.accent}; display: flex; gap: 35px; align-items: center; box-shadow: 0 35px 70px rgba(0,0,0,0.5); position: relative; overflow: hidden;">
                            <div style="position: absolute; top: -10px; right: -10px; background: ${styles.accent}; color: ${styles.bg.includes('f') ? '#000' : '#fff'}; padding: 12px 45px; font-weight: 950; font-size: 20px; transform: rotate(15deg); box-shadow: 0 8px 20px rgba(0,0,0,0.4);">DEAL</div>
                            <div style="width: 220px; height: 220px; border-radius: 35px; overflow: hidden; border: 7px solid #fff; background: #222; flex-shrink: 0; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                                <img src="${src}" style="width: 100%; height: 100%; object-fit: cover;"> 
                            </div>
                            <div style="flex: 1;">
                                <div style="font-size: 38px; font-weight: 950; color: #fff; margin-bottom: 15px; text-transform: uppercase; line-height: 1.1; letter-spacing: 1px;">${deal.name}</div>
                                <div style="font-size: 50px; font-weight: 950; color: ${styles.accent}; text-shadow: 2px 2px 0px rgba(0,0,0,0.3);">${formatPriceDisplay(deal.price)}</div>
                            </div>
                        </div>
                    `;
                }).join('');
                ...
                dealsHtml = `
                    <div style="padding: 100px 100px 60px 100px; background: ${styles.bg};">
                        <div style="font-weight: 950; font-size: 40px; color: ${styles.accent}; margin-bottom: 50px; text-transform: uppercase; letter-spacing: 12px; display: flex; align-items: center; gap: 35px;">
                            <div style="height: 5px; flex: 1; background: ${styles.accent}; border-radius: 3px;"></div>
                            EXCLUSIVE OFFERS
                            <div style="height: 5px; flex: 1; background: ${styles.accent}; border-radius: 3px;"></div>
                        </div>
            ...
            const categoryCards = Object.keys(grouped).map(cat => {
                const itemsList = grouped[cat].map((item: any) => {
                    ...
                    return `
                        <div style="margin-bottom: 25px; border-bottom: 2px dashed ${styles.border}; padding-bottom: 20px; display: flex; align-items: center; gap: 20px;">
                            ${imgSrc ? `
                            <div style="width: 70px; height: 70px; border-radius: 12px; overflow: hidden; border: 2.5px solid ${styles.accent}; flex-shrink: 0; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">
                ...
                return `
                    <div style="background: ${styles.cardBg}; border-radius: 40px; overflow: hidden; border: 3px solid ${styles.border}; margin-bottom: 40px; box-shadow: 0 20px 50px rgba(0,0,0,0.1); break-inside: avoid; display: flex; flex-direction: column;">
                        ${catImgSrc ? `<div style="height: 340px; width: 100%; overflow: hidden; background: #000;"><img src="${catImgSrc}" style="width: 100%; height: 100%; object-fit: cover; border-bottom: 8px solid ${styles.accent}"></div>` : ''}
                        <div style="padding: 40px;">
                            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 30px;">
                                <div style="height: 5px; flex: 1; background: ${styles.accent}; border-radius: 5px;"></div>
                                <div style="font-weight: 950; font-size: 34px; text-transform: uppercase; color: ${styles.accent}; letter-spacing: 4px;">${cat}</div>
                                <div style="height: 5px; flex: 1; background: ${styles.accent}; border-radius: 5px;"></div>
                            </div>
                            <div>${itemsList}</div>
                        </div>
            ...
            const footerHtml = `
                <div style="background: ${styles.darkerBg}; padding: 80px; text-align: center; border-top: 8px solid ${styles.accent};">
                    <div style="font-size: 28px; color: ${styles.mutedText}; font-weight: 900; letter-spacing: 10px; text-transform: uppercase; margin-bottom: 20px;">EXCELLENCE AT ${restaurantInfo.name}</div>
                    <div style="font-size: 16px; color: ${styles.mutedText}; opacity: 0.5; font-weight: 700;">Handcrafted Menu • Powered by SaySavor Pro Suite</div>
                </div>
            `;
            ...
            // 5. Capture & Save
            toast.loading("Rendering high-performance brochure...", { id: exportToast });
            await new Promise(resolve => setTimeout(resolve, 4500)); 

            const canvas = await html2canvas(ghost, {
                scale: 1.5, // Perfect balance for 2400px spread
            ...
            link.download = `${slugify(restaurantInfo.name)}_Brochure.${ext}`;
            link.click();

            document.body.removeChild(ghost);
            toast.dismiss(exportToast);
            toast.success("Final Refined Brochure Downloaded! 📥🗞️");

        } catch (error) {
            console.error("Export error:", error);
            toast.error("Failed to generate refined spread");
        }
    };
'''
...
print("Successfully applied Final Image Refinements and Balanced Heights")
